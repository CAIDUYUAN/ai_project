...
async function clearAll() {
  ...
  // 2. 로컬 메모리 삭제
  ['bm','cp','tg','yg','ts','nv','di'].forEach(pf => {
...
let SEL_PF = new Set(['bm','cp','tg','yg','ts','nv','di']); // 플랫폼 필터
...
function togglePlatform(pf) {
  ...
  if (SEL_PF.size === 0) SEL_PF = new Set(['bm','cp','tg','yg','ts','nv','di']); // 전부 해제 시 전체 선택
  renderOverview(); renderCompare(); renderCalendar(); renderDiagnosis();
}
  ...
  ['bm','cp','tg','yg','ts','nv','di'].forEach(pf => {
    ...
    filteredMonths.forEach(mo => {
      ...
      Object.entries(d.services).forEach(([name, s]) => {
        ...
        if (!svcMonthly[name]) svcMonthly[name] = [];
        svcMonthly[name].push({month:mo, count:s.count||0, fee:s.fee||0, delivery:s.delivery||0, ad:s.ad||0, total:s.total||0});
      });
    });
