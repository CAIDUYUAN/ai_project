...
function updateFileList() {
  const pfLabel = {bm:'배민', cp:'쿠팡', tg:'땡겨요'};
  const pfColor = {bm:'var(--grn)', cp:'var(--red)', tg:'#06D6A0'};
  const all = [
...
function aggregate(sel) {
  ...
  const dailyBM={}, dailyCP={}, dailyTG={};

  filtered.forEach(mo => {
    const bm=DB.bm[mo], cp=DB.cp[mo], tg=DB.tg[mo];
    if (bm) {
  ...
  return {
    ...
    days: new Set([...Object.keys(dailyBM),...Object.keys(dailyCP),...Object.keys(dailyTG)]).size,
    months, filtered, dailyBM, dailyCP, dailyTG, bep, bepMo,
...
function renderOverview() {
  ...
  set('k-del', W(ag.tDel)); set('k-del-sub', `배민 ${ag.bm.ord}건 + 쿠팡 ${ag.cp.ord}건`);
  ...
  // 일별 막대 차트
  const allDays = [...new Set([...Object.keys(ag.dailyBM), ...Object.keys(ag.dailyCP), ...Object.keys(ag.dailyTG)])].sort();
  const chart   = document.getElementById('bar-chart'); chart.innerHTML = '';
  const maxRev  = Math.max(...allDays.map(d => (ag.dailyBM[d]?.rev||0)+(ag.dailyCP[d]?.rev||0)+(ag.dailyTG[d]?.rev||0)), 1);
  allDays.forEach(day => {
    const bR=ag.dailyBM[day]?.rev||0, cR=ag.dailyCP[day]?.rev||0, gR=ag.dailyTG[day]?.rev||0;
    const tot = bR+cR+gR, H = Math.max(tot/maxRev*82, 2);
    const bH=tot?bR/tot*H:0, cH=tot?cR/tot*H:0, gH=H-bH-cH;
    const col = document.createElement('div'); col.className = 'bar-col';
    col.innerHTML = `<div class="bar-bm" style="height:${bH}px"></div><div class="bar-cp" style="height:${cH}px"></div><div style="width:100%;background:#06D6A0;min-height:${gR?2:0}px;height:${gH}px"></div><div class="bar-label">${parseInt(day.split('-')[2])}</div>`;
  ...
  ag.months.forEach(mo => {
    const bm=DB.bm[mo], cp=DB.cp[mo], tg=DB.tg[mo];
    const bmR=bm?.totalRev||0, cpR=cp?.totalRev||0, tgR=tg?.totalRev||0, tot=bmR+cpR+tgR;
    const totO   = (bm?.orders||0)+(cp?.orders||0)+(tg?.orders||0);
  ...
  if (ag.months.length > 1) {
    ...
    tr.innerHTML=`<td>합계</td><td style="color:#4ade80">${W(ag.bm.r)}</td><td style="color:#f87171">${W(ag.cp.r)}</td><td style="color:#06D6A0">${W(ag.tg.r)}</td><td>${W(ag.tR)}</td><td>${ag.tOrd}건</td><td class="neg">-${W(ag.tDeduct)}</td><td style="color:#4ade80;font-weight:700">${W(totDep)}</td><td class="${totPrf>=0?'pos':'neg'}">${W(totPrf)}</td>`;
    tbody.appendChild(tr);
  }
}
...
function renderCompare() {
  ...
  [{pf:'bm', name:'배달의민족', color:'#4ade80', dot:'var(--grn)', ...ag.bm},
   ...
   {pf:'tg', name:'땡겨요',     color:'#06D6A0', dot:'#06D6A0',   ...ag.tg},
    ...
    const prf = dep - fixedCost() * (ag.filtered.length||1) / (ag.tR ? 3 : 2);
  ...
  document.getElementById('cmp-tbody').innerHTML = [
    ['총 매출',         W(ag.bm.r),              W(ag.cp.r),              W(ag.tg.r),   W(ag.tR)],
    ['주문 건수',       `${ag.bm.ord}건`,         `${ag.cp.ord}건`,         `${ag.tg.ord}건`, `${ag.tOrd}건`],
    ['건당 평균',       W(ag.bm.ord?ag.bm.r/ag.bm.ord:0), W(ag.cp.ord?ag.cp.r/ag.cp.ord:0), W(ag.tg.ord?ag.tg.r/ag.tg.ord:0), W(ag.tOrd?ag.tR/ag.tOrd:0)],
    ['수수료',         W(ag.bm.fee),             W(ag.cp.fee),             W(ag.tg.fee), W(ag.tFee)],
    ['수수료율',        Pct(ag.bm.fee,ag.bm.r),   Pct(ag.cp.fee,ag.cp.r),   Pct(ag.tg.fee,ag.tg.r), Pct(ag.tFee,ag.tR)],
    ['배달비',         W(ag.bm.del),             W(ag.cp.del),             W(ag.tg.del), W(ag.tDel)],
  ...
  ].map(r=>`<tr><td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td style="font-weight:700">${r[4]}</td></tr>`).join('');
}
...
function renderCalendar() {
  ...
  const bmMo = DB.bm[`${y}-${mo}`], cpMo = DB.cp[`${y}-${mo}`], tgMo = DB.tg[`${y}-${mo}`];
  ...
  for (let d=1;d<=dim;d++) {
    ...
    const tot = bR+cR+gR, totO = bO+cO+gO;
    const cell = document.createElement('div');
    cell.className = 'cal-cell' + (tot>0?' has-data':'') + (ds===todayStr?' today':'');
    if (tot > 0) {
      const bw=bR/tot*100, cw=cR/tot*100, gw=100-bw-cw;
      cell.innerHTML = `<div class="cal-day">${d}</div><div class="cal-rev">${W(tot).replace('₩','')}</div><div class="cal-orders">${totO}건</div><div class="cal-bar"><div class="cal-bar-bm" style="width:${bw}%"></div><div class="cal-bar-cp" style="width:${cw}%"></div><div style="background:#06D6A0;width:${gw}%"></div></div>`;
      cell.addEventListener('mouseenter', () => showTooltip(cell,ds,bR,cR,bO,cO,bmMo,cpMo));
  ...
  renderCalSummary(y, m+1, bmMo, cpMo);
}
...
function showTooltip(cell, ds, bR, cR, bO, cO, bmMo, cpMo) {
  ...
  html += `<hr class="tt-divider"><div class="tt-row"><span style="font-weight:700">합계</span><span class="tt-val">${W(bR+cR)}</span></div><div class="tt-row"><span style="font-size:10px;color:var(--muted)">${bO+cO}건</span><span style="font-size:10px;color:var(--muted)">건당 ${W((bO+cO)?(bR+cR)/(bO+cO):0)}</span></div>`;
  tt.innerHTML = html;
...
function renderCalSummary(y, m, bmMo, cpMo) {
  const mo = String(m).padStart(2,'0');
  const tgMo = DB.tg[`${y}-${mo}`];
  ...
  document.getElementById('cal-tbody').innerHTML = `
    ...
    <tr class="tfoot"><td>합계</td><td>${bO+cO+gO}건</td><td>${W(bR+cR+gR)}</td><td class="neg">-${W(bFee+cFee+gFee)}</td><td style="color:var(--or)">-${W(bDl+cDl+gDl)}</td><td style="color:#f87171">-${W(bCpn+cCpn)}</td><td>${W((bO+cO+gO)?(bR+cR+gR)/(bO+cO+gO):0)}</td></tr>`;
}
